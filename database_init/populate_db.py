...
def populate_database():
    ...
    with app.app_context():
        ...
        try:
            with open('database_init/apn_data.csv', 'r', encoding='utf-8-sig') as f: # Use utf-8-sig
            ...
            logging.error("database_init/apn_data.csv not found. Cannot populate APN table.")
        except Exception as e:
            logging.error(f"An error occurred while reading database_init/apn_data.csv: {e}")

        
        # Populate CP table
        logging.info("Populating CP table...")
        try:
            with open('database_init/cp_data.csv', 'r', encoding='utf-8-sig') as f: # Use utf-8-sig
                reader = csv.DictReader(f)
                cp_count = 0
                for i, row in enumerate(reader):
                    try:
            ...
            logging.error("database_init/cp_data.csv not found. Cannot populate CP table.")
        except Exception as e:
            logging.error(f"An error occurred while reading database_init/cp_data.csv: {e}")
